...
plot(htree, labels = sample_info$Treatment, main = "Hierarchical Clustering Dendrogram")
rect.hclust(htree, k = length(unique(sample_info$Treatment)), border = 2:5)

# Check for NA or Infinite values and replace with zero
data[is.na(data)] <- 0
data[is.infinite(data)] <- 0

# Ensure data is a data frame and subset only numeric columns
...
# Perform PCA
pca <- prcomp(t(data_numeric), center = TRUE, scale. = TRUE)
summary(pca)
...
metadata <- sample_info  # Ensure sample_info has the necessary metadata
pca.dat <- cbind(pca.dat, metadata)

# Plot PCA with metadata groups
ggplot(pca.dat, aes(x = PC1, y = PC2, color = Treatment)) +
  geom_point() +
...
# Assuming log2_count_matrix is already calculated as per your code
boxplot(log2_count_matrix, outline = TRUE, main = "Boxplot of Log2-transformed Count Data",
        xlab = "Sample Name",
        ylab = "Log2-transformed Counts")

# Assuming log2_normalized_counts is already calculated as per your code
boxplot(log2_normalized_counts, outline = TRUE,
        main = "Boxplot of Log2-transformed Normalized Count Data",
        xlab = "Sample Name",
        ylab = "Log2-transformed Counts")
...
write.csv(top_gene_names, file = "Top_gene_names_IIb_vs_mock_20.csv", row.names = FALSE)
...
pheatmap(heatmap_data_ordered, 
         ...
         show_rownames = TRUE, 
         cluster_cols = FALSE,
         main = "Heatmap of rld for TOP 20 genes_names(IIb_vs_mock)" )

pheatmap(heatmap_data_ordered,
         main= "Heatmap of rld for TOP 20 cluster(IIb_vs_mock)")
...
# Create a heatmap of Z-scores with gene names as row names
rownames(zscore_subset) <- top1
#colnames(zscore_subset) <- custom_col_names
pheatmap(zscore_subset,
         main = "Heatmap of Z scores with gene names")
write.csv(zscore_subset,file = "407_Z_All_genes_Z_score_Heatmap.csv")
...
head(top_geneid)
YY <- top_geneid[ top_geneid$gene_name %in% top,]
missing_ids <- setdiff(top, YY$gene_name)
print(missing_ids)
...
pheatmap(zscore_subset,
         main = "Heatmap of Z scores with gene names")
write.csv(zscore_subset,file = "396_Z_All_genes_Z_score_Heatmap.csv")
...
#[3] "Treatment_MPXV.clade.IIb.infected_vs_mock"
...
resLFC_p_cut <- na.omit(resLFC_p_cut)


#####
write.csv(resLFC_p_cut, file = "resLFC_p_cut.csv")
# Write to CSV file
write.csv(resLFC_p_cut, "resLFC_p_cut_clean.csv", row.names = TRUE)
##### bubble


XX<-rownames(resLFC_p_cut1)
top <- XX$Gene
top <- as.character(XX)
#############################
top_geneid <- read.csv("ensemble_geneid.csv")
head(top_geneid)
YY <- top_geneid[ top_geneid$gene_id %in% top,]
# Filter top_geneid to only include rows where gene_id matches the top_UP_hits
top_gene_names1 <- top_geneid[top_geneid$gene_id%in% top, ]
#top_gene_names <- top_geneid[top_geneid$gene_id %in% top_UP_hits, ]
#top_gene_names$gene_name
top<-top_gene_names1$gene_id
top1<- top_gene_names1$gene_name
...
Heat <- resLFC[top,]
rownames(Heat) <- top1
write.csv(Heat,file = "resLFC_David_data.csv")
...
ggplot(resLFC,aes(x = log2FoldChange, y = -log10(padj))) +
  ...
  labs(
    title = "Volcano plot of Treatment_MPXV.clade.IIb_vs_mock", # Add the header here
    x = "Log2 Fold Change",
    y = "-log10(Padj)",
    color = "Regulation"
  ) +
...
####################################
KK <- read.csv("resLFC_p_cut.csv")
# Create a Volcano Plot
ggplot(KK, aes(x = log2FoldChange, y = -log10(padj))) +
  ...
  # Label specific points by their IDs
  geom_text(data = subset(KK, KK$ID %in% c("ENSG00000000419", "ENSG00000001461", "ENSG00000001629")), aes(label = ID), 
...
ggplot(resLFC, aes(x = log2FoldChange, y = -log10(padj))) +
  ...
  geom_vline(xintercept = c(-1, 1), linetype = "dashed", color = "black") +
  labs(
    title = "HeatMap of Treatment_MPXV.clade.I.infected_vs_mock",
    x = "Log2 Fold Change",
    y = "-log10(padj)",
    color = "Regulation"
  ) +
  scale_color_manual(values = c("Upregulated" = "red", "Downregulated" = "blue", "Not Significant" = "gray")) +
  theme_minimal() +
  geom_text(aes(label = ifelse(log2FoldChange > 1.0 & -log10(padj) > 1.3, row.names(resLFC),
                               ifelse(log2FoldChange < -1.0 & -log10(padj) > 1.3, YY$gene_id, ""))),
...
# Save the filtered data to a CSV file
write.csv(top_gene_names1, file = "UP_p.value_all_hits_gene_names_IIb_vs_mock_20.csv", row.names = FALSE)
write.csv(top_gene_names, file = "UP_p.value_20_hits_gene_names_IIb_vs_mock_20.csv", row.names = FALSE)


#up

#[1] "DNAJA1"   "PPP1R15A" "DDX5"     "HSPA8"    "ODC1"     "DUSP1"    "HSPH1"    "EGR1"    
#[9] "ZFP36"    "ATF4"     "SAT1"     "DNAJB1"   "IER3"     "DNAJA4"   "SQSTM1"   "DDIT4"   
#[17] "UBB"      "LAMB3"    "HSPA1B"   "HSPA1A" 

#top hits

#"GSTP1"   "DSP"     "HSPA8"   "EGR1"    "KRT17"   "DNAJB1"  "S100A11" "KRT1"    "SPRR1B" 
#[10] "KRT6C"   "KRT6B"   "KRT5"    "KRT10"   "KRT16"   "KRT14"   "IGFL1"   "S100A2"  "LAMB3"  
#[19] "HSPA1A"  "KRT6A"     

#down

# "PKP1"    "GSTP1"   "DSP"     "RPS12"   "KRT17"   "FGFBP1"  "S100A8"  "S100A11" "IVL"    
#[10] "KRT1"    "SPRR1B"  "KRT6C"   "KRT6B"   "KRT5"    "KRT10"   "KRT16"   "KRT14"   "IGFL1"  
#[19] "S100A2"  "KRT6A"  
...
# Save the filtered data to a CSV file
write.csv(top_gene_names1, file = "DOWN_p.value_all_gene_names_IIb_vs_mock_20.csv", row.names = FALSE)
write.csv(top_gene_names, file = "DOWN_p.value_20_gene_names_IIb_vs_mock_20.csv", row.names = FALSE)
...
# Save the filtered data to a CSV file
write.csv(top_gene_names1, file = "UP_padj_all_hits_gene_names_IIb_vs_mock_20.csv", row.names = FALSE)
write.csv(top_gene_names, file = "UP_padj_20_hits_gene_names_IIb_vs_mock_20.csv", row.names = FALSE)
...
Heat <- resLFC[top,]
rownames(Heat) <- top1
#write.csv(Heat,file = "396_Up_IIb_Heat_All_genes_Heatmap.csv")
...
# Save the filtered data to a CSV file
write.csv(top_gene_names1, file = "DOWN_padj_all_gene_names_IIb_vs_mock_20.csv", row.names = FALSE)
write.csv(top_gene_names, file = "DOWN_padj_20_gene_names_IIb_vs_mock_20.csv", row.names = FALSE)
############################################################################################
XX<-read.csv("407_down_Gene_names.csv")
top <- XX$ID
top <- as.character(top)
...
Heat <- resLFC[top,]
rownames(Heat) <- top1
write.csv(Heat,file = "407_IIb_down_Heat_All_genes_Heatmap.csv")
############################################################################


llb_gene <- read.csv("newly_ex.csv")
...
write.csv(geneid3 , file = "UP_newly_ex_padj_file.csv")
...
write.csv(res.df,file = "UP_newly_ex_gene_name.csv")

######################################################################################



llb_gene <- read.csv("Down_newly_ex.csv")
llb_geen1 <- llb_gene$gene_name

write.csv(Downregulated, file = "Upragulated_data_frame.csv")
up<-read.csv("Downragulated_data_frame.csv")
...
write.csv(geneid3 , file = "DOWN_newly_ex_padj_file.csv")
...
write.csv(res.df,file = "Down_newly_ex_gene_name.csv")
