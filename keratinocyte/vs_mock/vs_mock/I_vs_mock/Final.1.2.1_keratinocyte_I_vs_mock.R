...
setwd("D:/DEG/GSE219036_6_june/keratinocyte/vs_mock/I_vs_mock")

#load the count data

count_data <- read.csv("count_keratinocyte.csv", header=TRUE,row.names = 1)
count_data1 <- read.csv("count_keratinocyte.csv", header=TRUE)
colnames(count_data)
head(count_data)
...
# create histogram plot of p-values
hist(deseq_results_IIa_0.05$pvalue, breaks=seq(0, 1, length = 21), col = "grey", border = "white", 
...
Upregulated <- resLFC[resLFC$log2FoldChange > 1 & resLFC$padj < 1.30103, ]
...
# Save the filtered data to a CSV file
write.csv(top_gene_names1, file = "UP.all_hits_gene_names_I_vs_mock.csv", row.names = FALSE)
write.csv(top_gene_names, file = "UP.20_hits_gene_names_I_vs_mock.csv", row.names = FALSE)
...
Downregulated <- resLFC[resLFC$log2FoldChange < -1 & resLFC$padj < 1.30103, ]
...
# Save the filtered data to a CSV file
write.csv(top_gene_names1, file = "DOWN.all_gene_names_I_vs_mock.csv", row.names = FALSE)
write.csv(top_gene_names, file = "DOWN.20_gene_names_I_vs_mock.csv", row.names = FALSE)
