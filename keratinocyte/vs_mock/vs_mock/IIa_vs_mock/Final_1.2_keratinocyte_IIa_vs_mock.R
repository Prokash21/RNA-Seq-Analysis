...
setwd("E:/DWCT/DEG/GSE219036_6_june/keratinocyte/vs_mock/vs_mock/IIa_vs_mock")
...
#Step 1: filter based on p adjusted value 6739 
...
write.csv(top_gene_names, file = "Top_gene_names_IIa_vs_mock_20.csv", row.names = FALSE)
...
pheatmap(heatmap_data_ordered, 
         ...
         show_rownames = TRUE, 
         cluster_cols = FALSE,
         main = "Heatmap of rld for TOP 20 genes_names(IIa_vs_mock)" )

pheatmap(heatmap_data_ordered,
         main= "Heatmap of rld for TOP 20 cluster(IIa_vs_mock)")
...
ggplot(resLFC,aes(x = log2FoldChange, y = -log10(padj))) +
  ...
  labs(
    title = "Volcano plot of Treatment_MPXV.clade.IIa_vs_mock", # Add the header here
    x = "Log2 Fold Change",
    y = "-log10(padj)",
    color = "Regulation"
  ) +
...
# Save the filtered data to a CSV file
write.csv(top_gene_names1, file = "UP_p.value_all_hits_gene_names_IIa_vs_mock.csv", row.names = FALSE)
write.csv(top_gene_names, file = "UP_p.value_20_hits_gene_names_IIa_vs_mock.csv", row.names = FALSE)



#up

#[1] "DNAJA1"   "PPP1R15A" "HSP90AB1" "DDX5"     "HSPA8"    "ODC1"     "HSPH1"    "EGR1"    
#[9] "SAT1"     "DNAJB1"   "DNAJA4"   "MCL1"     "UBC"      "SQSTM1"   "UBB"      "FOS"     
#[17] "JUN"      "NRARP"    "HSPA1B"   "HSPA1A"  
#top hits

#"GSTP1"    "DNAJA1"   "PPP1R15A" "HSP90AB1" "HSPA8"    "ODC1"     "HSPH1"    "EGR1"    
#[9] "KRT17"    "SAT1"     "DNAJB1"   "DNAJA4"   "MCL1"     "UBC"      "SQSTM1"   "S100A11" 
#[17] "KRT1"     "UBB"      "FOS"      "SFN"     

#down

# "GSTP1"   "DSP"     "GAPDH"   "KRT17"   "FGFBP1"  "S100A11" "KRT1"    "SPRR1B"  "KRT6C"  
#[10] "SFN"     "LGALS7B" "KRT6B"   "KRT5"    "KRT10"   "KRT16"   "KRT14"   "IGFL1"   "S100A16"
#[19] "S100A6"  "KRT6A" 
...
# Save the filtered data to a CSV file
write.csv(top_gene_names1, file = "DOWN_p.value_all_gene_names_IIa_vs_mock.csv", row.names = FALSE)
write.csv(top_gene_names, file = "DOWN_p.value_20_gene_names_IIa_vs_mock.csv", row.names = FALSE)
...
# Save the filtered data to a CSV file
write.csv(top_gene_names1, file = "UP_padj_all_hits_gene_names_IIa_vs_mock.csv", row.names = FALSE)
write.csv(top_gene_names, file = "UP_padj_20_hits_gene_names_IIa_vs_mock.csv", row.names = FALSE)
...
Heat <- resLFC[top,]
rownames(Heat) <- top1
write.csv(Heat,file = "396_Upa_IIa_Heat_All_genes_Heatmap.csv")
#up
...
# Save the filtered data to a CSV file
write.csv(top_gene_names1, file = "DOWN_padj_all_gene_names_IIa_vs_mock.csv", row.names = FALSE)
write.csv(top_gene_names, file = "DOWN_padj_20_gene_names_IIa_vs_mock.csv", row.names = FALSE)
...
Heat <- resLFC[top,]
rownames(Heat) <- top1
write.csv(Heat,file = "407_IIa_down_Heat_All_genes_Heatmap.csv")
