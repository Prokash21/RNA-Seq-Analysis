...
setwd("D:/DEG/GSE219036_6_june/keratinocyte/vs IIa/I")
...
dds$Treatment <- relevel(dds$Treatment , ref = "MPXV clade IIa infected")
dds$Treatment
...
write.csv(normalize_counts,"normalized_counts_keratinocyte_I_vs_IIa.csv")
...
deseq_results_IIa <- results(dds, contrast=c("Treatment", "MPXV clade I infected", "MPXV clade IIa infected"))
deseq_results_IIa <- as.data.frame(deseq_results_IIa)
head(deseq_results_IIa)
deseq_results_IIa_0.05 <- results(dds,contrast=c("Treatment", "MPXV clade I infected", "MPXV clade IIa infected"), alpha = 0.05)
...
write.csv(deseq_results,"deseq_result_keratinocyte_I_vs_IIa.csv")
write.csv(filtered,"filtered_keratinocyte_I_vs_IIa.csv")
...
pheatmap(sampleDistMatrix,clustering_distance_rows=sampledists,
         ...
         main = "Heatmap of sample-to-sample distance matrix for I_vs_IIa")
...
top_hits
write.csv(top_hits,"top_hits_I_vs_IIa_20.csv")
class(top_hits)
...
pheatmap(assay(rld)[top_hits, ], 
         ...
         fontsize_row = 8,
         annotation_col = df,
         main = "Heatmap of rld for TOP 20 genes_ids(I_vs_IIa)")
...
write.csv(top_gene_names, file = "Top_gene_names_I_vs_IIa_20.csv", row.names = FALSE)
...
pheatmap(heatmap_data_ordered, 
         ...
         show_rownames = TRUE, 
         cluster_cols = FALSE,
         main = "Heatmap of rld for TOP 20 genes_names(I_vs_IIa)" )

pheatmap(heatmap_data_ordered,
         main= "Heatmap of rld for TOP 20 cluster(I_vs_IIa)")
...
ggplot(resLFC,aes(x = log2FoldChange, y = -log10(pvalue))) +
  ...
  labs(
    title = "HeatMap of Treatment_MPXV.clade.I.infected_vs_MPXV.clade.IIa.infected", # Add the header here
    x = "Log2 Fold Change",
    y = "-log10(P-Value)",
    color = "Regulation"
  ) +
...
# Save the filtered data to a CSV file
write.csv(top_gene_names1, file = "UP.all_hits_gene_names_I_vs_IIa_20.csv", row.names = FALSE)
write.csv(top_gene_names, file = "UP.20_hits_gene_names_I_vs_IIa_20.csv", row.names = FALSE)


#up

#[1] "MSMO1"  "SCD"    "NEFM"   "RPS12"  "TPT1"   "FGFBP1" "RHCG"   "RPS11"  "FDPS"   "IVL"   
#[11] "S100A9" "IGFN1"  "TM4SF1" "KRT6C"  "CALML5" "KRT6B"  "MT1X"   "IGFL1"  "IGFL3"  "RPL12" 

#top hits

#"KLF6"     "HSP90AB1" "GADD45B"  "CRYAB"    "RSP12"    "ODC1"     "DNAJB1"   "TPT1"    
#[9] "RPS11"    "UBC"      "IGFN1"    "RRAD"     "CXCL8"    "TSPYL2"   "KRT6B"    "KRT5"    
#[17] "KRT14"    "IGFL1"    "HSPA1B"   "HSPA1A"  

#down

# "PFKP"    "KLF6"    "GADD45B" "CRYAB"   "ODC1"    "VGF"     "ASS1"    "KLF10"   "ANXA5"  
#[10] "RRAD"    "CXCL8"   "HSPA6"   "CYB5D1"  "TSPYL2"  "FAM110C" "PIM3"    "MT-ND6"  "SNHG32" 
#[19] "HSPA1B"  "HSPA1A" 
...
# Save the filtered data to a CSV file
write.csv(top_gene_names1, file = "DOWN.all_gene_names_I_vs_IIa_20.csv", row.names = FALSE)
write.csv(top_gene_names, file = "DOWN.20_gene_names_I_vs_IIa_20.csv", row.names = FALSE)
...
DEresults <- results(dds, contrast = c("Treatment", "MPXV clade I infected", "MPXV clade IIa infected"))
...
top_geneid1 <- read.csv("ensemble_geneid.csv")
# Filter top_geneid to only include rows where gene_id matches the top_hits
top_gene_names1 <- top_geneid[top_geneid1$gene_id %in% genesOfInterest, ]
top_gene_names1 <- as.character(top_gene_names1$gene_name)


goResults <- gost(query = top_gene_names1, 
                  organism = 'hsapiens')

goResults1 < as.data.frame(goResults)

goResults1<-goResults$result
