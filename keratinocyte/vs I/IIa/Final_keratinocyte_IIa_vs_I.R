...
#########################################
deseq_results_test <- results(dds, contrast=c("Treatment", "MPXV clade IIa infected", "MPXV clade I infected"))
summary(deseq_results_test)
...
summary(deseq_results_IIa_0.05)
deseq_results_IIa_0.05 <- as.data.frame(deseq_results_IIa_0.05)
head(deseq_results_IIa_0.05)
summary(deseq_results_IIa_0.05)
# Identify the name of the outlier sample
outlier_sample <- "sample_name_of_outlier"

# Remove the outlier sample from the DESeqDataSet
dds <- dds[!(colnames(dds) == outlier_sample)]

# Set cooksCutoff to a higher value (e.g., 1) to be less stringent
deseq_results_IIa_0.05 <- results(dds, contrast = c("Treatment", "MPXV clade IIa infected", "MPXV clade I infected"), alpha = 0.05, cooksCutoff = 1)

# Set independentFiltering to FALSE to disable independent filtering
deseq_results_IIa_0.05 <- results(dds, contrast = c("Treatment", "MPXV clade IIa infected", "MPXV clade I infected"), alpha = 0.05, independentFiltering = FALSE)
