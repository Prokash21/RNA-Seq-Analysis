...
setwd("D:/DEG/GSE219036_6_june/colon organoids/vs_mock/venn_colon_all")

# Load your DESeq2 result files
cladeI_vs_mock <- read.csv("UP.all_hits_gene_names_I_vs_mock.csv")
cladeIIa_vs_mock <- read.csv("UP.all_hits_gene_names_IIa_vs_mock_20.csv")
...
common_genes <- Reduce(intersect, list(upregulated_genes_cladeI, upregulated_genes_cladeIIa))
...
write.csv(common_genes_df, "common_genes_colon.csv", row.names = FALSE)
