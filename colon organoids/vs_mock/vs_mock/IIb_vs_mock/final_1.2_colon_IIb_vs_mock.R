...
setwd("E:/DWCT/DEG/GSE219036_6_june/colon organoids/vs_mock/vs_mock/IIb_vs_mock")
...
write.csv(normalize_counts,"normalized_counts_colon_IIb_vs_mock.csv")
...
write.csv(deseq_results,"deseq_result_colon_IIb_vs_mock.csv")
write.csv(filtered,"filtered_colon_IIb_vs_mock.csv")
...
data.matrix <- count_data1
data.matrix <- matrix(nrow = 36601,ncol = 12)
colnames(data.matrix) <- c(
+  paste("mock",1:3,sep = ""),
+  paste("clade IIa",1:3,sep = ""),
+  paste("clade IIb",1:3,sep = ""),
+  paste("clade I",1:3,sep = ""),
)

rownames(data.matrix) <- paste("gene_id",1:36601,sep = "")
# Perform PCA on the raw count data
pca_raw <- prcomp(assay(dds))

plot(pca_raw$x[,1], pca_raw$x[,2], 
     pch=20, col=as.numeric(dds$Cell_type),
     xlab="PC1", ylab="PC2",
     main="PCA of Raw Count Data")
text(pca_raw$x[,1], pca_raw$x[,2], labels=row.names(pca_raw$x))


#Perform PCA
pca_result <- prcomp(count_data, scale = TRUE)

# Access the PCA scores for the samples
pca_scores <- pca_result$x
summary(pca_scores)
# Plot the PCA
plot(pca_scores[, 1], pca_scores[, 2], pch = 20,
     xlab = "Principal Component 1 (PC1)", ylab = "Principal Component 2 (PC2)",
     main = "PCA of 12 Samples")
####################################################
...
# PCA plot
plotPCA(pca_raw,intgroup=c("Cell_type", "Treatment"))
